...
class BlockchainController {
  ...
  constructor(app, blockchainObj) {
    ...
    //added after Udacity reviewer comments - missing endpoint
    this.validateChain();
  }
  ...
  //endpoint added for this.validChain() endpoint after Udacity reviewer comments
  validateChain() {
    //using recommended get method | example from submitStar to pull in /validateChain from blockchain.js file
    this.app.get("/validateChain", async (req, res) => {
      let vChain = await this.blockchain.validateChain();
      if (vChain.length > 0) {
        return res.status(404).send("Invalid chain");
      } else {
        return res.status(200).send("Valid chain <3");
      }
    });
  }
}
